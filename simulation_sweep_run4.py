...
def systemic_shock(network, eta, p0, xi):
    ...
    for node in network.nodes:
        ...
        if bank.balance_sheet_size < eta:
            bank.update_insolvency()
            print(f"Bank {node} has become insolvent due to balance sheet size decreasing from customers switching to other banks")
            if random.random() < xi:
                bank.update_bankruptcy()
                print(f"Bank {node} has become bankrupt due to systemic shock")
