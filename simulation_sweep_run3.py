...
bailout_probabilities = np.linspace(0, 1, 23)  # Bailout probabilities to test
...
# Print out every point on each graph
for prob in bailout_probabilities:
    x = prob
    y_grid = avg_adj_utility_runs_grid[prob]
    y_small_world = avg_adj_utility_runs_small_world[prob]
    y_power_law = avg_adj_utility_runs_power_law[prob]
    print(f'Bailout Probability: {x:.2f}, Grid: {y_grid:.2f}, Small World: {y_small_world:.2f}, Power Law: {y_power_law:.2f}')
...
plt.plot(bailout_probabilities, [avg_adj_utility_runs_power_law[prob] for prob in bailout_probabilities], label='power_law',marker='s')
