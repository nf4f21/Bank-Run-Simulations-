...
def run_simulation(grid_size, rewire_prob, num_nodes, powerlaw_exponent, eta, p0, xi, k_threshold, c0, c1, theta,bailout_probability ,num_timesteps):
    ...
    return time_steps, avg_risk_strategy_runs_grid, avg_risk_strategy_runs_small_world, bankrupt_banks_runs_grid, bankrupt_banks_runs_small_world,cascade_sizes_grid, cascade_sizes_small_world, binned_sizes_grid, binned_freq_grid, binned_sizes_small_world, binned_freq_small_world
...
time_steps, avg_risk_strategy_runs_grid,avg_risk_strategy_runs_small_world,bankrupt_banks_runs_grid,bankrupt_banks_runs_small_world, cascade_sizes_grid,cascade_sizes_small_world,binned_sizes_grid, binned_freq_grid, binned_sizes_small_world, binned_freq_small_world= run_simulation(
    ...
    bailout_probability=0.5,
    num_timesteps=100  # Number of times the entire simulation should be run.
)
...
# Prepare the data for plotting for each network type and plot them
def prepare_and_plot(cascade_sizes, network_type):
    # Ensure cascade_sizes is a Counter object
    cascade_sizes = Counter(cascade_sizes)
    
    # Get the sizes and frequencies
    sizes, frequencies = zip(*cascade_sizes.items())

    # Perform logarithmic binning
    binned_sizes, binned_frequencies = logarithimic_binning(cascade_sizes)

    # Create a log-log plot
    plt.figure(figsize=(10, 6))
    plt.loglog(sizes, frequencies, marker='o', linestyle='none', label=f'{network_type} Network - Raw')
    plt.loglog(binned_sizes, binned_frequencies, marker='s', linestyle='none', label=f'{network_type} Network - Binned')

    # Set labels and title
    plt.xlabel('Cascade size')
    plt.ylabel('Frequency')
    plt.title(f'Distribution of Bankruptcy Cascades - {network_type} Network')
    plt.legend()
...
prepare_and_plot(cascade_sizes_grid, "Grid")
prepare_and_plot(cascade_sizes_small_world, "Small-World")
