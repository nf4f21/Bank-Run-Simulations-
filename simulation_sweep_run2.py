...
# Now you would call this function inside a loop over bailout probabilities to generate the plot
...
'''
# Sample data
original_utilities = [avg_adj_utility_runs_grid[prob] for prob in bailout_probabilities]  # Your original utilities

# Let's say we want to maintain the end point as is and start the first point at 0
start_shift = original_utilities[0]  # Amount to shift the first point to 0
end_shift = 0  # We do not want to shift the end point

# Generate a list of shift amounts that linearly interpolate between start_shift and end_shift
shift_amounts = np.linspace(start_shift, end_shift, len(original_utilities))

# Shift the original utilities by the calculated amounts
shifted_utilities = [utility - shift for utility, shift in zip(original_utilities, shift_amounts)]

# Now plot the original and the shifted data
plt.plot(bailout_probabilities, original_utilities, label='Original Grid', marker='o')
plt.plot(bailout_probabilities, shifted_utilities, label='Shifted Grid Start to 0', marker='o', linestyle='--')


plt.xlabel('Bailout Probability (q)')
plt.ylabel('Average Adjusted Utility')
plt.title('Grid Network Transformations')
plt.legend()
plt.grid(True)
plt.show()
