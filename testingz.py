...
def create_bank_network(grid_size, rewiring_probability, num_nodes, powerlaw_exponent):
    ...
    for network in (grid_network, small_world_network, power_law_network):
        for node in network.nodes:
             network.nodes[node]['bank'] = Bank(risk_strategy=random.random(), financial_manager=financial_manager, network_degree=0, tau=0.5)
...
def simulate_customer_reactions(customer_network, bank_network):
    for customer_node in customer_network.nodes():
        customer = customer_network.nodes[customer_node]['customer']
        if customer.trust_level < customer.bank.tau:  # tau is the threshold for losing trust
            amount_withdrawn = customer.withdraw_savings()
            customer.bank.handle_withdrawal(amount_withdrawn)
            customer.bank.respond_to_customer_behavior(1)  # Process one withdrawal
...
def run_simulation_with_sweep(grid_size, rewire_prob, num_nodes, powerlaw_exponent, eta, p0, xi, k_threshold, c0, c1, theta,num_timesteps):
    ...
    bankrupt_banks_power_law_each_run = []
    ...
    for bailout_probability in bailout_probabilities:
        ...
        for time_step in range(num_timesteps):
            ...
            state = capture_network_state(grid_network)
            avg_risk_strategies.append(state['average_risk_strategy'])
            total_liabilities_list.append(state['total_liabilities'])
        ...
        # At the point where you have the 'Additional Strategy State'
        additional_strategy_state_power_law = capture_network_state(power_law_network)
        bankrupt_banks_power_law_each_run.append(additional_strategy_state_power_law['bankrupt_banks'])


        # At the end of your function, return the collected data
    return time_steps, avg_risk_strategy_runs_grid, avg_risk_strategy_runs_small_world,avg_risk_strategy_runs_power_law ,bankrupt_banks_runs_grid, bankrupt_banks_runs_small_world,cascade_sizes_grid, cascade_sizes_small_world, binned_sizes_grid, binned_freq_grid, binned_sizes_small_world,bailout_probabilities, bankrupt_banks_power_law_each_run,avg_risk_strategies,total_liabilities_list
...
time_steps, avg_risk_strategy_runs_grid,avg_risk_strategy_runs_small_world,avg_risk_strategy_runs_power_law,bankrupt_banks_runs_grid,bankrupt_banks_runs_small_world, cascade_sizes_grid,cascade_sizes_small_world,binned_sizes_grid, binned_freq_grid, binned_sizes_small_world, binned_freq_small_world,bankrupt_banks_power_law_each_run,avg_risk_strategies,total_liabilities_list= run_simulation_with_sweep(
...
# Now you can use bankrupt_banks_power_law_each_run to plot your graph
plt.figure(figsize=(10, 5))
plt.plot(range(1, len(bankrupt_banks_power_law_each_run) + 1), bankrupt_banks_power_law_each_run, marker='o', linestyle='-', color='red')
plt.title('Number of Bankrupt Banks in Power-Law Network After Each Run')
plt.xlabel('Run Number')
plt.ylabel('Number of Bankrupt Banks')
plt.grid(True)
plt.show()

plt.figure(figsize=(10, 5))
plt.plot(range(1, len(avg_risk_strategies) + 1), avg_risk_strategies, marker='o', linestyle='-', color='blue')
plt.title('Average Risk Strategy per Run for Grid Network')
plt.xlabel('Run Number')
plt.ylabel('Average Risk Strategy')
plt.grid(True)
plt.show()

plt.figure(figsize=(10, 5))
plt.plot(range(1, len(total_liabilities_list) + 1), total_liabilities_list, marker='o', linestyle='-', color='green')
plt.title('Total Liabilities per Run for Grid Network')
plt.xlabel('Run Number')
