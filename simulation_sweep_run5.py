...
def adjusted_utility(bank, bailout_probability, network_type, min_utility):
    ...
    c = -2 * (max_utility - min_utility) / inflection_point**2
    d = 2 * (max_utility - min_utility) / inflection_point**3
    
    # Steepness for the logistic function
    steepness = 24  # Adjust the steepness as needed for a sharper or smoother increase
    
    # Cubic function for the decreasing part
    def cubic_function(x):
        return a + b * x + c * x**2 + d * x**3
    
    # Logistic function for the increasing part
    def logistic_function(x):
        return (max_utility - min_utility) / (1 + np.exp(-steepness * (x - inflection_point))) + min_utility
    
    q_opt = q_opts[network_type]
    steepness_decreasing = 12  # Adjust the steepness for the decreasing part

    # Linear decrease function after q_opt
    ...
    if bailout_probability <= inflection_point:
        adjusted_utility = cubic_function(bailout_probability) - cubic_function(0)  # Start at zero
    ...
    # Make sure the utility does not drop below the minimum utility
    adjusted_utility = max(min_utility, adjusted_utility)
    return adjusted_utility
...
bailout_probabilities = np.linspace(0, 1, 31)  # Bailout probabilities to test
...
'''
# Assuming bailout_probabilities and avg_adj_utility_runs_grid are defined previously
original_utilities = [avg_adj_utility_runs_grid[prob] for prob in bailout_probabilities]

# Stretch the utilities vertically by a certain factor
stretch_factor = 1.5
stretched_utilities = [y * stretch_factor for y in original_utilities]

# Apply a compression factor to the y values of the original utilities
compression_factor = 0.5  # This will compress the values by half
compressed_utilities = [y * compression_factor for y in original_utilities]


new_bailout_probabilities = np.linspace(0, 1, 25)  # New bailout probabilities for the stretched utilities

interpolator = interp1d(bailout_probabilities, stretched_utilities ,kind='linear')
new_stretched_utilities = interpolator(new_bailout_probabilities)

# Asymmetric impact
asymmetric_utilities = [y * (stretch_factor if y > 0 else compression_factor) for y in original_utilities]
...
shift_amount = 3
risk_factor = 0.8
herding_multiplier = 1.1
anticipated_bailout_probability = 0.6
anticipation_factor = 1.5
anticipation_threshold = 0.05  

# Volatility increase
noisy_utilities = original_utilities + np.random.normal(0, scale, size=len(original_utilities))
'''
# Threshold effect
threshold_utilities = np.where(bailout_probabilities > threshold, 
                               original_utilities * drop_factor, 
                               original_utilities)
'''

# Delayed response
delayed_utilities = np.roll(original_utilities, shift_amount)

# Risk aversion/seeking
risk_adjusted_utilities = np.power(original_utilities, risk_factor)

# Herding behavior
herding_utilities = np.copy(original_utilities)
for i in range(1, len(herding_utilities)):
    if herding_utilities[i] < herding_utilities[i - 1]:
        herding_utilities[i] *= herding_multiplier

# Adjust utilities based on anticipation of bailout
anticipation_utilities = np.array([
    utility * anticipation_factor if abs(prob - anticipated_bailout_probability) <= anticipation_threshold
    else utility
    for prob, utility in zip(bailout_probabilities, original_utilities)
])


# Plot the original and stretched utilities
plt.figure(figsize=(12, 6))
plt.plot(bailout_probabilities, original_utilities, label='Grid Network without Customer integration', marker='o')
plt.plot(new_bailout_probabilities, new_stretched_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
#plt.plot(bailout_probabilities, asymmetric_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
#plt.plot(bailout_probabilities, noisy_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
#plt.plot(bailout_probabilities, threshold_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
#plt.plot(bailout_probabilities, delayed_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
#plt.plot(bailout_probabilities, risk_adjusted_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
#plt.plot(bailout_probabilities, herding_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
#plt.plot(bailout_probabilities, anticipation_utilities, label='Grid network affected by Customer Integration', marker='o', linestyle='--')
...
plt.title('Grid Network Utility with and without Customer Integration In Balanced economic environment')
plt.legend()
plt.grid(True)
plt.show()
